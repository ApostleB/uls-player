<script lang="ts">
  ...
  $effect(() => {
    ...
    untrack(() => {
      ...
      // 행을 누르는 것은 듣겠다는 뜻으로 본다. 예전에는 일부러 자동
      // 재생을 하지 않았고 그 근거도 위 주석에 남아 있었는데, 실사용에서
      // "눌렀는데 안 울려서 클릭이 씹힌 줄 알았다"로 드러나 뒤집었다.
      //
      // paused를 true로 한 번 튕겼다가 false로 되돌린다. Svelte의
      // $state는 같은 값을 다시 대입하는 걸 변화로 보지 않으므로, 그냥
      // `paused = false`만 두면 이미 false인 경우(아래 설명) 아무
      // effect도 다시 돌지 않는다 — 이게 이 이중 대입이 존재하는 이유다.
      // audio.play()를 직접 부르지는 않는다 — 재생은 bind:paused가
      // 소유하고, 여기서 직접 부르면 그 바인딩이 반응형 상태와 어긋난다.
      //
      // 왜 필요한가: 이미 재생 중이던 행에서 다른 행으로 넘어가는
      // 경우(가장 흔한 사용 패턴 — 목록을 훑으며 계속 듣는다)에는
      // paused가 이미 false다. 이때 src가 바뀌면 브라우저가 이벤트 없이
      // 실제 media.paused를 true로 되돌리므로(위 paused 선언부 주석),
      // 우리 쪽 반응형 상태는 false로 멈춰 있고 실제 재생은 멈춘 채로
      // 남는다 — canplay가 오면 bind:paused가 이 불일치를 실제
      // 값(true)쪽으로 맞춰버려 재생이 저절로 다시 시작되지도 않는다
      // (실측: 뮤테이션 검증 참고). true로 튕겼다가 false로 되돌리면
      // 값 자체는 그대로지만 그 사이 실제로 변화가 있었으므로 effect가
      // 다시 스케줄되고, 다시 실행될 때는 최신 값(false)과 그 시점의
      // 실제 media.paused를 비교해 필요하면 play()를 다시 부른다 —
      // 그래서 이미 재생 중이었어도, 방금 막 멈춰 있었어도 항상 같은
      // 하나의 경로로 새 행이 재생을 시작한다.
      paused = true;
      paused = false;
    });
