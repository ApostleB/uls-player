...
export default defineConfig({
	server: {
		watch: {
			...
			//
			// 재현 결과(2026-09-08): data/·media/·.superpowers/·test-results/에
			// touch·신규 파일 추가·(atomic.ts와 동일한) tmp-write+rename까지
			// 실제로 걸어 봤지만 이 네 디렉터리는 재시작도 리로드도 유발하지
			// 않았다 — 즉 여기 배제는 실증된 원인 제거가 아니라 예방 차원이다
			// (media/는 11GB라 감시 자체가 리소스 낭비이기도 하다).
			//
			// 실제로 재현에 성공한 원인은 이 네 디렉터리가 아니라
			...
			ignored: ['**/data/**', '**/media/**', '**/.superpowers/**', '**/test-results/**']
		}
	},
	plugins: [
		tailwindcss(),
